...
export function reduceTask(task: TaskRecord, event: TaskEvent): TaskRecord {
  ...
  if (event.type === 'DOCUMENT_REBOUND') return { ...bump(task), boundDocumentId: event.documentId, controlledReloadAt: null, stableSince: event.now, lastProgressAt: event.now, lastObservationAt: event.now };
  if (event.type === 'PAUSE') return { ...bump(task), state: 'PAUSED', pauseReason: event.reason, statusDetail: event.detail ?? null };
  if (event.type === 'RESUME') return { ...bump(task), state: 'WAITING_ANSWER', pauseReason: 'NONE', statusDetail: null };
