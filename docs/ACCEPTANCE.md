...
Status: v0.2.17 has 188 automated tests using mocked Chrome APIs and jsdom fixtures. Recovery cases cover a delayed final probe finding a recovered or busy page, fresh stability before a single send, changed identity/drafts, missing-evidence diagnostics, unavailable/old documents, freshest verification evidence and pending-send uncertainty. The earlier supplied composer DOM was also checked offline: adjacent version/Pro spans yield `6 pro` and the stop control remains busy. The full supplied HTML is not included in the repository. This does not validate live long-running background reliability. Check the latest local validation result or CI before relying on a build.

v0.2.17: the latest field log establishes completed-answer recognition and a confirmed follow-up, followed by a site error and a refreshed view showing the consumed answer without the accepted user turn. Regression tests replay that sequence and verify the specific mismatch diagnostic, up to three automatic refreshes, natural recovery when the submitted turn reappears, and explicit one-use continuation from the displayed answer. Popup/source/revision checks, drafts, changed identity, busy/error states, pending uncertainty, deadline and send limits are covered. An adapter/coordinator/editor fixture verifies the explicit action leads to exactly one further click, preserves the ledger/budget and finishes at the existing cap. Typecheck, all 188 tests and build pass locally. Browser runtime startup is unavailable in this session; no live research-page send, stop or refresh was used for validation. Live installed-extension/background acceptance remains pending.
...
v0.2.16: the supplied log and read-only live DOM confirmed two action bars in one paired turn: the first belongs to the user, the second to the assistant. The old first-match logic missed completed responses. A sanitized DOM-structure capture (no conversation text, original IDs or URLs) is retained in tests/fixtures/new-paired-turn.html. Tests now exercise ownership, order, hidden controls, split bars, earlier assistant segments, stop precedence and stable continuation; a joined fixture runs the actual adapter, coordinator and editor through two sends, refresh, deduplication and final limit. Probe failures separated by fresh bound-page observations no longer accumulate. Mutation bursts are sampled every 250 ms without delaying explicit reads or starving continuous progress. The reproduced failures were observed before their fixes. Typecheck and 160 automated tests pass. Browser tooling denied access to chrome://extensions, so installation/reload and live extension-driven two-round background acceptance remain unverified; no stop, send or refresh was performed on the research conversation.
